...
class _StationMapScreenState extends ConsumerState<StationMapScreen> {
  ...
  bool _useMockData = true; // Start with mock data by default
  ...
  void initState() {
    ...
    _userLocation = const LatLng(37.7749, -122.4194); // San Francisco

    WidgetsBinding.instance.addPostFrameCallback((_) {
      _initializeMap();
      _updateMarkersWithMockData(); // Load mock stations immediately
    });
  }

  Future<void> _initializeMap() async {
    try {
      // Try to fetch real recommendations in background
      ref.read(recommendationProvider.notifier).fetchRecommendations();
    } catch (e) {
      // Already using mock data, so no additional action needed
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Using offline mode. Connect to see live data.'),
  ...
  void _updateMarkersWithMockData() {
    ...
    final nearestStations = MockStationsData.getNearestStations(
      ...
      limit: 12, // Show all 12 mock stations
    );
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          // Google Map - Always show map (with mock data by default)
          GoogleMap(
                  ...
                  children: [
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          const Text(
                            'Demo Mode - 12 Swap Stations Near You',
                            style: TextStyle(
                              color: Colors.white,
                          ...
                          Text(
                            'Tap any marker to view details and get directions',
                            style: TextStyle(
